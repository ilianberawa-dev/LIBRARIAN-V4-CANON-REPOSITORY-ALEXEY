...
**База:** CLAUDE.md один — рекомендательный (~40-60% соблюдения), у Алексея ушло из контекста за пару ходов.
...
5. **3 темы (`dizain`/`arhitektura`/`prochee-idei`)** — выбраны Алексеем как 3 главные сферы текущей работы. Расширим если появится новая.
...
- ❌ **PreToolUse hook (блокирующий 100%)** — пользователь решил не рисковать ("если может всё сломать — пока тех долг")
...
2. **TG bot setup (Триггер 3)** — Алексей должен создать бота через `@BotFather`, добавить `TG_BOT_TOKEN` + `TG_CHAT_ID` в GitHub Secrets. Инструкция: `.github/workflows/SETUP-TG-NOTIFY.md`
3. **PowerShell desktop shortcut (Триггер 2)** — Алексей запускает `scripts/inbox-tools/setup-inbox-shortcut.md` инструкцию на Windows
...
- **"Репо для всех областей жизни"** — Алексей хочет расширить за пределы AI/Realty. Сравнение с claude.ai Projects: claude.ai Projects = одно место, шарят всю папку, нет hooks/skills, нет git. Наш репо = git + hooks + версионирование, но требует терминал. Гибрид: claude.ai Projects смотрит в этот же репо через файл-загрузку.
- **Переписать `~/.bashrc` `sync` алиас** — пользователь отказался от удаления `upload-design`, но `sync` нужно проверить что делает `git pull && ls inbox/`
...
1. Подождать первый "тестовый" заход Алексея — он обещал. Не делать структурных изменений до этого
2. Если Алексей подтвердит работоспособность — обсудить идею "репо для всех областей жизни" (предложить: либо отдельный репо с тем же шаблоном, либо подпапка `life/` здесь)
